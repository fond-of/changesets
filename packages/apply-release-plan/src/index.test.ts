    ...
    it("should update a changelog for one package", async () => {
      ...
      let { changedFiles } = await testSetup(
        ...
        releasePlan.getReleasePlan(),
        {
          ...releasePlan.config,
          changelog: [
            path.resolve(__dirname, "test-utils/simple-get-changelog-entry.ts"),
            null,
          ],
        }
      );
        ...
        try {
          await applyReleasePlan(
            ...
            await getPackages(tempDir),
            {
              ...releasePlan.config,
              changelog: [
                path.resolve(__dirname, "test-utils/failing-functions.ts"),
                null,
              ],
            }
          );
  ...
  it("should get the commit for an old changeset", async () => {
    ...
    let { tempDir } = await testSetup(
      ...
      releasePlan.getReleasePlan(),
      {
        ...releasePlan.config,
        commit: [
          path.resolve(__dirname, "test-utils/simple-get-commit-entry.ts"),
          null,
        ],
        changelog: [
          path.resolve(__dirname, "test-utils/simple-get-changelog-entry.ts"),
